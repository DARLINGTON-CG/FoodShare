...
@injectable
class CreateMessagesBloc extends Bloc<CreateMessagesEvents, CreateMessagesState> {
  final IMessageRepository _messageRepository;

  CreateMessagesBloc(this._messageRepository) : super(CreateMessagesState.initial()) {
  ...
  void _onMessageChanged(MessageChanged event, Emitter<CreateMessagesState> emit) {
    emit(state.copyWith(
        data: state.data.copyWith(
          messages: MessageList<Message>(event.message.map((MessagePrimitive primitive) => primitive.toDomain())),
          ),
  ...
  void _onSaved(Saved event, Emitter<CreateMessagesState> emit) async {
    ...
    if (state.data.failureOption.isNone()) {
      final Either<MessageFailure, Unit>? failureOrSuccess =  await _messageRepository.send(state.data);
      emit(state.copyWith(
          isSaving: false,
